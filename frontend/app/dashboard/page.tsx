...
function Dashboard() {
    ...
    const getDashBoardData = async () => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/shared/dashboard/${localStorage.getItem('id')}`, {
                method: 'GET',
                headers: {
            ...
            if (!response.ok) {
                ...
                setError(data?.error || "An error occurred during fetching dashboard data")
                return
            }
            setDashboardData(data)
        } catch (error: any) {
            console.error("Error during signin:", error);
            setError(error?.error || "An error occurred during fetching dashboard data")
        } finally {
            setIsLoading(false)
        }
        ...
        <HomeLayout>
            {isLoading ?
                ...
                <div className='flex flex-col gap-10'>
                    <div>
                        <Label className='text-2xl font-bold'>{greeting}, {dashboardData?.username}</Label>
                    </div>
                    <div>
                        <Label className='text-xl font-bold'>Monthly Summary</Label>
                        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mt-2'>
                            <Card className='flex flex-col items-center gap-3'>
                                <p className='font-medium text-xl text-green-600'>{currency}{dashboardData?.totalIncome}</p>
                            ...
                            <Card className='flex flex-col items-center gap-3'>
                                <p className='font-medium text-xl text-red-600'>{currency}{dashboardData?.totalExpense}</p>
                                <div className='flex gap-2'>
                                    <div className='flex items-center '>
                                        <svg xmlns="http://www.w3.org/2000/svg" className='dark:text-white' viewBox="0 0 24 24" width="20" height="20" color="#000000" fill="none">
                            ...
                            <Card className='flex flex-col items-center gap-3'>
                                <p className='font-medium text-xl text-blue-600'>{currency}{dashboardData?.balance}</p>
                                <div className='flex gap-2'>
                            ...
                            <Card className='flex flex-col items-center gap-3'>
                                <p className='font-medium text-xl text-orange-600'>{dashboardData?.transactionsCount}</p>
                                <div className='flex gap-2'>
                                    <div className='flex items-center '>
                                        <svg xmlns="http://www.w3.org/2000/svg" className='dark:text-white' viewBox="0 0 24 24" width="20" height="20" color="#000000" fill="none">
                    ...
                    <div>
                        ...
                        <div className='grid grid-cols-1 md:grid-cols-2 gap-5 w-full mt-2'>
                            <Card className='h-fit'>
                                <Label className='text-sm font-normal'>Income vs. Expenses</Label>
                                <BarChart />
                            </Card>
                            <Card className='h-fit'>
                                <Label className='text-sm font-normal'>Expense Categories</Label>
                                <DonutChart />
                            </Card>
                            <Card className='h-fit'>
                                <Label className='text-sm font-normal'>5-Day Expense Comparison</Label>
                                <LineChart />
                            </Card>
                            <Card className='h-fit'>
                                <Label className='text-sm font-normal'>Daily Expense Heatmap</Label>
                                <HeatMapChart />
                            </Card>
                        </div>
                    </div>
