...
function Expenseview() {
  ...
  useEffect(() => {
    ...
    if (data) {
      const parsedData: Expense = JSON.parse(decodeURIComponent(data));
      setExpense(parsedData);
    }
  }, [searchParams]);
  ...
  const { id, date, category, description, amount } = expense;
  ...
  const handleEditChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement> | { target: { name: string; value: string } }) => {
    ...
    setEditData((prev: Expense) => ({ ...prev, [name]: value }));
  };
  ...
  const deleteExpense = async (event: any) => {
    ...
    setIsSubmitting(true)
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/expenses/delete/${id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
    ...
    <HomeLayout>
      <div className='space-y-10'>
        <div className='flex justify-end'>
          <div className='flex gap-3'>
            <Button type='button' onClick={confirmEditExpense}>
              <div>
            ...
            <Button type='button' onClick={() => setDeletes(true)}>
              <div>
                <svg xmlns="http://www.w3.org/2000/svg" className='dark:text-white' viewBox="0 0 24 24" width="24" height="24" color="#ffffff" fill="none">
      ...
      <Modal Title="Edit Expense" isOpen={edit} onClose={() => { setEdit(false), setIsSubmitting(false) }} buttonText="Update" onSubmit={editExpense} isLoading={isSubmitting} loadingText='updating'>
        ...
        <div className="flex flex-col gap-1">
          ...
          <Select options={expenses} value={category} onChange={(value) => handleEditChange({ target: { name: 'category', value } })} />
        </div>
        <div className="flex flex-col gap-1">
...
export default withAuth(Expenseview);
